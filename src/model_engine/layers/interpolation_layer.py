...
from server_config import generator, media, fps
...
class InterpolationLayer(BaseLayer):
    ...
    def __init__(self, product_type):
        ...
        self.output_gifs_path = media['output_gifs'][product_type]
        self.fps = fps
        self.generator_manager = GeneratorManager()
        self.generator = self.generator_manager.generator
    ...
    def make_latent_interp_animation(self, interpolation_dict, interpolation_type):
        ...
        for alpha in tqdm(amounts):
            ...
            images = self.generator.generate_image_from_z(interpolated_latent_code)
        ...
        save_name = self.output_gifs_path/'latent_space_traversal.gif'
    ...
    def get_image_and_associated_latent_code(self, proportion_list):
        ...
        image = Image.fromarray(self.generator.generate_image_from_z(latent_code)[0]).resize((self.results_size, self.results_size))
        return image, latent_code

    def transform(self, interpolation_type):
        image_1, latent_code_1 = self.get_image_and_associated_latent_code(self.proportion_list_1)
        image_2, latent_code_2 = self.get_image_and_associated_latent_code(self.proportion_list_2)
        interpolation_dict = {
            'image_1': image_1,
