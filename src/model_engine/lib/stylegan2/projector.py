...
class Projector:
    ...
    def set_network(self, Gs, minibatch_size=1):
        ...
        self._dlatents_var = tf.Variable(
            tf.zeros([self._minibatch_size] + list(self._dlatent_avg.shape[1:])), name='dlatents_var'
        )
        ...
        self._dlatents_expr = tf.tile(
            self._dlatents_var + dlatents_noise, [1, self._Gs.components.synthesis.input_shape[1], 1]
        )
    ...
    def start(self, target_images):
        ...
        if sh[2] > self._target_images_var.shape[2]:
            ...
            target_images = np.reshape(
                target_images, [-1, sh[1], sh[2] // factor, factor, sh[3] // factor, factor]
            ).mean((3, 5))
